...
async function simulate() {
  ...
  // user1 redeems 100 shares
  await waitForEnter("user1 redeems 100 shares");
  await (await vaultU1.redeem(tc100, user1.address, user1.address)).wait();
  await printState();

  // user2 redeems 25 shares
  await waitForEnter("user1 redeems 25 shares");
  await (await vaultU2.redeem(tc25, user2.address, user2.address)).wait();
  await printState();
